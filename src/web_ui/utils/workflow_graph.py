...
class WorkflowNode:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.start_time and self.end_time:
            result["data"]["duration"] = round((self.end_time - self.start_time) * 1000, 2)  # milliseconds
...
class WorkflowGraphBuilder:
    ...
    def add_thinking_node(self, parent_id: str, content: str, model_name: Optional[str] = None) -> str:
        """Add a thinking/reasoning node."""
        node_id = self._generate_node_id()
        
        # Calculate position based on parent
        parent_node = self._get_node_by_id(parent_id)
        y_pos = parent_node.position["y"] + self.vertical_spacing if parent_node else self.vertical_spacing
    ...
    def add_error_node(self, parent_id: str, error: Exception | str) -> str:
        ...
        error_msg = str(error) if isinstance(error, str) else f"{type(error).__name__}: {str(error)}"
    ...
    def add_end_node(self, parent_id: str, final_result: Optional[str] = None) -> str:
        """Add the ending node."""
        node_id = self._generate_node_id()
